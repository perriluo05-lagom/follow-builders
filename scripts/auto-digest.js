...
async function generateDigest(feedData, config) {
  ...
  if (config.language === 'zh') {
    ...
    // 构建精简的指令（整合所有 prompt 规则，压缩 token）zhi'jie
    const systemPrompt = `用中文生成AI行业简报，包含三类内容：X平台博主推文摘要、AI企业博客摘要、播客精华改写
    专业术语 (AI、LLM、GPU、RAG、prompt、agent 等) 保留英文；人名、公司、产品名、URL 原样保留；行文流畅，风格专业接地气，杜绝空洞套话，不编造信息，所有条目必须附带原始来源链接
    分模块要求
    X推文摘要：作者全名（备注id）；提炼实质观点、行业分析、产品信息，且逻辑链条清晰，完整说明观点始末，让人仅看摘要就能清楚地理解核心信息，若涉及确实上下文的摸棱两可表述，因自行稍作补充；正文部分无需再次重复“作者表示”之类描述，直接进入正题；每位作者摘要均需分段，且每段后附推文链接，并用简单分割线区分不同博主
    博客摘要：标题格式为‘博客名称：文章标题’，100-300字；优先核心发布、数据、新功能，摘录原文引言，标注落地影响；最后附原文链接
    播客摘要：分为三个板块，标题→开头用一段话介绍播客背景、主要内容与核心观点→无序列表分段逐一陈列关键洞察并予以说明（逻辑链条清晰，完整说明始末）；优先反直觉观点，可引用原文金句；独立成文，不要 “本期节目” 这类表述；最后附单集视频链接
