...
async function generateDigest(feedData, config) {
  ...
  if (config.language === 'zh') {
    ...
    分模块要求
    X推文摘要：作者全名（备注id）；提炼实质观点、行业分析、产品信息，且逻辑链条清晰，完整说明观点始末，让人仅看摘要就能清楚地理解核心信息；每位作者摘要均需分段，且每段后附推文链接，并用简单分割线区分不同博主
    博客摘要：标题格式为‘博客名称：文章标题’，100-300字；优先核心发布、数据、新功能，摘录原文引言，标注落地影响；最后附原文链接
    播客摘要：分为三个板块，标题→开头一段介绍核心观点→无序列表分段逐一陈列关键洞察（逻辑链条清晰，完整说明始末）；优先反直觉观点，可引用原文金句；独立成文，不要 “本期节目” 这类表述；最后附单集视频链接
